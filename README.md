...
### Exercise 4 (L5): Wells and Deep N-Well

<img src="images/missing.png" width=0>

Problem 4a shows a nwell region next to a pwell region.
Set DRC style to `drc(full)` since most well-checks are compute extensive and off with `drc(fast)`.
Paint `ntap` into `nwell`, then add `nsubstratencontact` contact.
Fix remaining spacing, overlap and width rules until DRC is clean.

Problem 4b shows a `pwell` region with `psubstrate` inside it.
A `psubstratepcontact` is missing, add it.
Fix remaining spacing, overlap and width rules until DRC is clean.

Problem 4c shows a `dnwell` (deep-nwell).
Fix the min. width error first.
The remove the regions left over from problem 4b, since these are causing spacing rule violations.
Then add a `nwell` ring around the `dnwell`.
Finally add a tap contact to the `nwell` region.
Usually a guard ring would be placed around the `dnwell` region.
There is a parameterized device for the guard ring which should be used in practice.

### Exercise 5: Derived Layers

Derived layers when working with transistors, because Magic does merge some layers in some cases
and show virtual layers in other cases, for ease of use.
E.g. when `poly` crosses `ndiff` Magic automatically shows a `nmos` region in the overlap region.

To show some of the derived layers use the following Tcl commands in Magic.
...
# To reset the view to the defaults
% feedback clear
```

<img src="images/missing.png" width=0>

The derived layers are generated by Magic automatically and are correct in most cases.

Poly contacts also require a special `NPC` layer for contact cuts.

### Exercise 6: Parameterized and PDK Devices

<img src="images/missing.png" width=0>

Magic starts with subcells unexpanded. Type `x` to expand cells.
To expand all cells put the pointer into empty space, hit `s` to select everything, then hit `x`.

Problem 6a shows a parameterized nmos device. Why does it show DRC errors?
Minimum area error for metal1. Of no consequence when connection to metal 1 is made when routing.

Select a cell instance by typing `i`, then type `>` to decend into a selected cell.
When decending into the cell the DRC error is still there.

Problem 6b shows a transistor (esdfet for high voltage) with an angled gate.
Design rules do not allow angled edges, for reasons.
Magic can deal with this by anstracting the cell, using an abstract view, .maglef file extenstion.
Select the cell by typing `i` then type `cellname filepath ...` to see the filesystem path of the cell file.
Change the cells path to the current dir with `cellname filepath .`, decend into the cell with `>` then save.
This creates a local copy of the cell.
Delete the angled poly edges now until DRC is clean.
Ascend back up by typing `<`.

Problem 6c shows a cell from the primitive device library.
There are some DRC errors related to 45 degree angles.

### Exercise 7 (L8): Angle Error and Overlap Rules

<img src="images/missing.png" width=0>

Problem 7a  
It's hard to impossible to draw outside the grid in Magic.
Can happend when crossing polygons, resulting in a off-grid intersection.
There is a drawn grid, a snap-grid and a internal grid.
See `scalegrid` command.

Flip a selected area with command `side`.

Problem 7b  
Cannot have angled corners on local interconnect.

Problem 7c  
Angled area is not exactly 45 degress, hard to see by eye.

Problem 7d  
Poly overlaps diffusion here, but each is in a different cell.
What is poly and diffusion from different cells overlap?
Magic does not allow this! Problematic since other tools may do this.

Contacts from different cells are allowed to overlap, but must overlap exactly.

Problem 7e
Looks ok, but is not.
Two overlapping cells at play here.
Via contacts need to be placed differently in hierarchical cell as compared to flat cell.

### Exercise 8: Unimplemented Rules

Problem 8a  
<img src="images/missing.png" width=0>
Seal ring cell goes around perimeter of chip, protects from sawing and dicing stress.
Special rules only for seal ring, very special rules which do not apply normally.
Not worth implementing all those special rules for the seal ring, just accept it as is.
Use abstract view for the seal ring to be able to work with it without getting DRC issues.
Seal ring can only be created with the seal ring generator.

Generate seal ring from command line, giving it width, height and output dir as arguments.
...
Separate launch script for Magic, with different tech file, which shows a more exact GDS representation.
...
Still some undefined layers, not all layers defined by the Skywater process.

Klayout (object based) can also be used to see all the layers, useful or not.

Problem 8b  
<img src="images/missing.png" width=0>
If a GDS file is referenced in a cell (shown by typing `property`),
that GDS will be used when Magic writes out GDS, not what is seen in Magic.
SRAM cell, comprised of large uniform arrays of the same geometries, can produce fine without drop in yield.

### Exercise 9: Latch-Up

Problem 9a
<img src="images/missing.png" width=0>
Latch-up check, will error if distance to tap is too high.

Use `getcell` command to add a tap cell.
```shell
% getcell sky130_fd_sc_hd__tapvpwrvgnd_1_0
```

### Exercise 10: Antenna Rules

<img src="images/missing.png" width=0>

Antenna check is a electrical rule check (ERC).
...
Connect a diode to the long stretch of metal1 to fix the antenna error.
Alternatively fix the antenna violation by closing the route on a metal1.

### Exercise 11: Density Rules

<img src="images/missing.png" width=0>

Big design 700 x 700 um, this box size is used by Skywater to do the density check.
metal1 only a thin ring around the perimeter, low density.
metal2 is filling in most of the area, high density.

To check coverage (density) use the `cif cover` command.
```shell
% cif cover MET1
% cif cover MET2
```

There is a script for density checking in the PDK.

Create a GDS file from the example
```
...
Density can be fixed by another script
```shell
...
Result is a new GDS file with 

Look at the new GDS file in Magic
```shell
% gds read exercise_11_fill_pattern.gds
```

Separate layer for fill pattern, show with `see m2fill` command in Magic.

In practice need to merge design with fill pattern. Positioning is important!
```shell
% box values 0 0 0 0
...
In practice hard to get that much metal, need to fix by stripping metal from design if there is too much.

Do exercise_12.mag, reduce DRC errors down to 0 by applying everything learned so far.
